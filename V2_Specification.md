...
На этом экране пользователь видит все поля данные присущие папке (ресурсу). Я перечислил их выше. 
Тут они представлены в виде списка полей (для редактирования) и текстов (информация).
В этой "карточке" ресурса пользователь не может поменять для ресурса его тип, его дату создания, счисло найденных медиафайлов. Но остальное доступно для редактирования.
В этом окне пользователь может определить для папки (ресурса) длительность алайдшоу, а так же какие типы медиафайлов будут учитываться в этой папке.
...
Это небольшой диало скнопками "применить и отмена", в котором пользователь может 
...
- текстовое поле "Часть наименования". Оно пустое, но если пользователь введет тут какой то текст, то нужно отобрать только ресурсы в которых либо в коротком наименовании, либо в полном адресе встречается этот тест (как участок) без учета регистра букв.

При закрытии этого диалога по кнопве "Применить"- сортировка и фильтры применяются к списку ресурсов на основном экране. При примении фильтра на этом экране внизу появляется предупреждение с тексом описания примененного фильтра.
...
Настройки в виде закладок разделены на "Общие", "Настройки сортировки", "Настройки слайдшоу"

1. Общие

1.1 В общих настройках поля "Пользователь по-умолчанию", "Пароль по-умолчанию". Если они не пустые для папки (ресурса), то программа применяет их.

1.2 Кноппка вызова диалога получения прав "на работу с локальными файлами андроид".
- После нажатия и работы кнопки у пользователя запрашивается разрешение на перезапуск программы

1.3 Кнопка вызова диалога получения прав "на работу с внешней сетью SFTP".
- После нажатия и работы кнопки у пользователя запрашивается разрешение на перезапуск программы
